 ...
 * context-aware recommendations for SHL assessments.
 */
...
class RAGService {
  constructor(geminiApiKey, openaiApiKey, pineconeApiKey) {
    this.geminiService = new GeminiService(geminiApiKey);
    this.embeddingService = new EmbeddingService();
  }

  /**
   * Get assessment recommendations using RAG approach
  ...
  async getRecommendations(query, timeLimit = 60, maxResults = 10) {
    try {
      ...
      console.error('Error in RAG recommendation process:', error);
      throw new Error(`Failed to get RAG recommendations: ${error.message}`);
    }
  }

  /**
   * Initialize the RAG service by ensuring the vector index exists
   * @returns {Promise<void>}
   */
  async initialize() {
    try {
      await this.embeddingService.ensureIndexExists();
      console.log('RAG service initialized successfully');
    } catch (error) {
      console.error('Error initializing RAG service:', error);
      throw new Error(`Failed to initialize RAG service: ${error.message}`);
    }
  }
}
